...
async function getTrainers(){
    ...
    const rawFrontPicTables = await fetch(`https://raw.githubusercontent.com/${repo}/src/data/trainer_graphics/front_pic_tables.h`)
    const textFrontPicTables = await rawFrontPicTables.text()
    ...
    const trainersSpriteConversionTable = await getTrainersSpriteConversionTable(textFrontPicTables, textGraphicsTrainers)

    await regexTrainersParties(textTrainersParties, await regexTrainers(textTrainers, trainersSpriteConversionTable))
}
...
async function buildScriptsObjs(){
    ...
    await getItemsIcon()
    // await getHiddenItems() // not used?
    await getHeldItems()
    // await getTutorItems() // too much of a pain, still have hoenn map name
    await bugFixTrainers()

    Object.keys(items).forEach(itemName => {
        if (items[itemName]["description"].includes("?????") && itemName != "ITEM_NONE"){
        ...
        Object.keys(trainers[zone]).forEach(trainer => {
            ...
            if(localStorage.getItem(sprite)){
                ...
                if(sprites[sprite].length < 500){
                    ...
                    spriteRemoveBgReturnBase64(sprite, `https://raw.githubusercontent.com/${repo}/${sprite}`)
                }
            }
        })
    })
...
function getTrainerSpriteSrc(trainerSprite){
    const url = `https://raw.githubusercontent.com/${repo}/${trainerSprite}`
